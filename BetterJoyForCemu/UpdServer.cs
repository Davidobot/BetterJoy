    ...
    class UdpServer {
        ...
        public void Start(IPAddress ip, int port = 26760) {
            if (!Boolean.Parse(ConfigurationManager.AppSettings["MotionServer"])) {
                form.console.AppendText(I18n.Str("MsgMotionServerOff"));
                return;
            }
            ...
            try { udpSock.Bind(new IPEndPoint(ip, port)); } catch (SocketException ex) {
                ...
                form.console.AppendText(I18n.Str("MsgServerStartFailed"));
                return;
            }
            ...
            running = true;
            form.console.AppendText(I18n.Format("MsgStartingServer", ip.ToString(), port));
            StartReceive();
        }
